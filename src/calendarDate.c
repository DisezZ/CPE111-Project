...
*/
int checkDatePast(checkDay,checkMonth,checkYear)
{
...
int validDatePast(char inputDate[])
	{
	int valid = 1;
	...
	if ( difference == 1 || difference == 0) /*diference == 1; mean future and diffence == 0; mean today*/
		{
		valid = 0;
		}
...
int checkDates(char inputDate[])
	{
	int valid = 0; /*for check valid (1 is true, 0 is false)*/
	
	removeTerminator(inputDate);
	valid = validDateCharacter(inputDate); /*connect to function validDateCharacter and send argument inputDate then return valid*/
	if (valid == 1)
		{
		valid = validDate(inputDate);
		}
	if (valid == 1)
		{
		valid = validDatePast(inputDate);
		}
	return valid;
	}
